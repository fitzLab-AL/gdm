...
#'
#' @usage gdm.varImp(spTable, geo, splines = NULL, knots = NULL, fullModelOnly = FALSE,
#' nPerm = 50, parallel = FALSE, cores = 2, sampleSites = 1, sampleSitePairs = 1, outFile = NULL)
#' %- maybe also 'usage' for other objects documented here.
#'
#' @param spTable A site-pair table, same as used to fit a \code{\link{gdm}}.
#'
#' @param geo Similar to the \code{\link{gdm}} geo argument. The only difference is that the geo argument does not have a default in this function.
#'
#' @param splines Same as the \code{\link{gdm}} splines argument.
#'
#' @param knots Same as the \code{\link{gdm}} knots argument.
#'
#' @param fullModelOnly Set to TRUE to test only the full variable set. Set to FALSE to estimate model significance and variable importance and significance using matrix permutation and backward elimination. Default is FALSE.
#'
#' @param nPerm Number of permutations to use to estimate p-values. Default is 50.
#'
#' @param parallel Whether or not to run the matrix permutations and model fitting in parallel. Parallel processing is highly recommended when either (i) the nPerms argument is large (>100) or (ii) a large number of site-pairs (and or variables) are being used in model fitting (note computation demand can be reduced using subsampling - see next arguments). The default is FALSE.
#'
#' @param cores When the parallel argument is set to TRUE, the number of cores to be registered for parallel processing. Must be <= the number of cores in the machine running the function.
#'
