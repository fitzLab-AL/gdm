    ...
    if (is.null(knots)) {
        ...
        if (is.null(splines)) {
            ...
            if (length(splines) != nPreds) {
                stop(paste("Number of splines does not equal the number of predictors. Splines argument has", length(splines), "items but needs", nPreds, "items."))
            }
        ...
        if (is.null(splines)) {
            ...
            {
                index <- i * 3
                if ((knots[index - 1] < knots[index - 2]) || (knots[index] < knots[index - 2]) || (knots[index] < knots[index - 1])) {
...
plot.gdm <- function (x, plot.layout=c(2,2), plot.color="blue", plot.linewidth=2.0, ...) {
    options(warn.FPU = FALSE)
...
predict.gdm <- function (object, data, ...) {
    options(warn.FPU=FALSE)
...
summary.gdm <- function (object, ...) {
    print("", quote=FALSE)
    print("", quote=FALSE)    
    print("GDM Modelling Summary", quote=FALSE);
    print(paste( "Creation Date: ", object$creationdate ), quote=FALSE);
    print("", quote=FALSE)    
    # call <- match.call()
    m <- match.call(expand.dots=FALSE)
    print(paste("Name: ", m[[2]]), quote=FALSE)
    print("", quote=FALSE)    
    print(paste("Data: ", object$dataname ), quote=FALSE)
    print("", quote=FALSE)    
    print(paste("Samples: ", object$sample ), quote=FALSE)
    print("", quote=FALSE)    
    print(paste( "Geographical distance used in model fitting? ", object$geo ), quote=FALSE)
    print("", quote=FALSE)    
    print(paste("NULL Deviance: ", object$nulldeviance ), quote=FALSE)
    print(paste("GDM Deviance: ", object$gdmdeviance ), quote=FALSE)
    print(paste("Deviance Explained: ", object$explained ), quote=FALSE)
    print("", quote=FALSE)    
    print(paste("Intercept: ", object$intercept ), quote=FALSE)
    print("", quote=FALSE)
    thiscoeff <- 1
    thisquant <- 1
    for (i in 1:length(object$predictors)) {
        print(paste("Predictor ", i, ": ", object$predictors[[i]], sep=""), quote=FALSE)            
        print(paste("Splines: ", object$splines[[i]], sep=""), quote=FALSE)
        numsplines <- object$splines[[i]]
        for (j in 1:numsplines) {
            if (j == 1) {
                print(paste("Min Knot: ", object$knots[[thisquant]], sep=""), quote=FALSE)          
            } else if (j == numsplines) {
                print(paste("Max Knot: ", object$knots[[thisquant]], sep=""), quote=FALSE)
            } else {
                print(paste(round(100 / (numsplines - 1), digits=2), "% Knot: ",object$knots[[thisquant]], sep=""), quote=FALSE)
            }
            thisquant <- thisquant + 1
        }
        for (j in 1:numsplines) {
            print(paste("Coefficient[", j, "]: ", object$coefficients[[thiscoeff]], sep=""), quote=FALSE)
...
formatsitepair <- function(bioData, bioFormat, dist="bray", abundance=F, 
    ...
    # species data as x,y,species list
        ...
        if (class(predData) == "RasterStack" | class(predData) == "RasterLayer" | class(predData) == "RasterBrick") {
            ...
            if (siteColumn %in% colnames(newDataTable)) {
                # empty
            } else {
...
createsitepair <- function(dist, spdata, envInfo, dXCol, dYCol, siteCol, weightsType, custWeights) {
    ...
    if ((siteCol %in% colnames(envInfo)) == TRUE) {
    ...
    if ((dXCol %in% colnames(envInfo)) == TRUE) {
        if ((siteCol %in% colnames(envInfo)) == TRUE){
    ...
    } else if ((dXCol %in% colnames(spdata)) == TRUE) {
