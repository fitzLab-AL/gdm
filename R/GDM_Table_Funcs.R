...
plotUncertainty = function(spTable, leaveOut, bsIters, geo=FALSE, splines=NULL, 
  ...
  if(parallel==TRUE){
    ...
    ##first removes a number of sites according to input
    subSamps <- lapply(1:bsIters, removeSitesFromSitePair, tab=spTable, leaveOut=leaveOut)
    ##models the subsamples
  ...
  #checkModel1 <- gdm(spTable, geo=T, splines=splines, knots=knots)
  
  #checkModel2 <- gdm(spTable, geo=F, splines=splines, knots=knots)
...
singleSppSitePair <- function(bioData, bioFormat, siteColumn=NULL, XColumn, 
  ...
  ##sppID = id to select which species to build site-pair tabel for 
  ...
  ##custWeights = custom weights vector
  ...
  ##if bioFormat is not an acceptable number, exit function
  if(bioFormat %in% c(1:2)){} else{
    stop("Acceptable values for the bioFormat argument are: 1 or 2")
  }
  ##if samples is not a number, then exit function
  if(is.null(samples)==FALSE){
    if(is.numeric(samples)==FALSE | samples<0){
  ...
  if(weightType %in% c("equal", "custom")){} else{
    stop("Acceptable values for the weightType argument are: equal, or custom")
  }
  ##if weightType == custom, makes sure a custWeights is attached
  if(weightType=="custom" & is.null(custWeights)==T){
    stop("custom weightType provided with no custWeights")
  ...
  if(bioFormat==1 | bioFormat==2){
    ...
    ##filters out species data, by the species to be modeled
    ##prep for filtering
    siteCol <- which(colnames(bioData)==siteColumn)
    sppDat <- bioData[,-c(siteCol, xCol, yCol)]
    headDat <- bioData[,c(siteCol)]
    sppDat[sppDat>=1]=1
    sppDat[sppDat==0]=0
    bioData <- cbind(as.data.frame(headDat), sppDat)
    colnames(bioData)[1] <- siteColumn
    
    ##removes random samples 
    if(is.null(samples)==FALSE){
      if(nrow(bioData)<samples){
        warning("Fewer records remaining than specified in samples, continuing without sampling")
      }else{
        randRows <- sample(1:nrow(bioData), samples)
        ...
        ##checks to see if the selected samples still has data for determined species
        sppCol <- bioData[,which(colnames(bioData)==sppID)]
        if(length(which(sppCol==1))==0){
          stop("no records with desired species left in bioData after random samples removed")
        }else if(length(which(sppCol==1))<5){
          warning("less than five records with desired species left in bioData after random samples removed")
        }
      } 
    }
  ...
  ##creates distance matrix
  sppCol <- bioData[,which(colnames(bioData)==sppID)]
  dissimMat <- matrix(0, nrow=length(sppCol), ncol=length(sppCol))
  
  ##indentifies presence and absence sites
  presSites <- which(sppCol==1)
  absSites <- which(sppCol==0)
  ##changes dissimularity matrix into binary
  ##pres->pres=1
  ##pres->abs=0
  ##abs->abs= -1, and then removed after site-pair table created   
  for(r in 1:length(sppCol)){
    for(c in 1:length(sppCol)){
      if((r %in% presSites & c %in% presSites) & r!=c){
        dissimMat[r,c] <- 1
      }else if((r %in% absSites & c %in% absSites) & r!=c){
        dissimMat[r,c] <- -1
        #toRemove <- unique(c(toRemove, r))
      }
    }
  }
  ##reassembles into distance object
  distData <- as.dist(dissimMat)
  
  outTable <- as.data.frame(createsitepair(dist=distData, spdata=bioData, envInfo=predData, dXCol=XColumn, dYCol=YColumn, siteCol=siteColumn, weightsType=weightType, custWeights=custWeights))
  #outTable <- outTable[outTable$distance!=-1,]
